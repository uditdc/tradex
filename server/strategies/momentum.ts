...
/**
 * The original strategy: candle-derived indicators only (EMA stack, RSI, ATR%, volume
 * ratio, swing support/resistance, regime tag). `state` is whatever the route passes as
 * `systemOne`'s `state` — this module only cares that it includes `indicators`.
 */
export function buildQuestions(): StrategyQuestions {
  return {
    ...
    factorMeta: [
      { key: 'trend', label: 'Trend', kind: 'directional' },
      { key: 'momentum', label: 'Momentum', kind: 'directional' },
      { key: 'levels', label: 'Levels', kind: 'directional' },
      { key: 'volatility', label: 'Volatility', kind: 'conviction' },
      { key: 'volume', label: 'Volume', kind: 'conviction' },
